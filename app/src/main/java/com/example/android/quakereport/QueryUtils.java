    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i < feature.length(); i++) {
                ...
                JSONObject currentEarthquakes = feature.getJSONObject(i);
                JSONObject properties = currentEarthquakes.getJSONObject("properties");

                //其实可以直接写成getString的形式，虽然他是double类型，但是我们get其实是得到之后的形式，即String
                //String magnitude = properties.getString("mag");
                //这种方法是在我不知道可以直接提取成字符串的时候做了一个格式转换，但是其实是不需要的
                ...
                //这里我们对ms时间进行了格式转换，但是放在这里其实不好，因为我们还要显示时间，如果在这里转变就需要改变
                //Earthquakes的结构，在其内部添加日期和时间两个变量，所以我们可以考虑去getView中修改显示格式
                //同时要注意，如果我们这里采用String格式的time，则会发生 long -> String -> long ->String
                //如果我们把Earthquake的String改成long，则只需要在getView中进行一次改变就可以
                earthquakes.add(new Earthquake(magnitude, place, time, url));

                //Log.v("Earthquake",String.valueOf(properties.getDouble("mag"))+properties.getString("place")+ properties.getString("time"));
            }

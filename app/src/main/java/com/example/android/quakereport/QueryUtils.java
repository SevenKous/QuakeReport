 ...
 * 这里我们都是用的static变量、方法，因为我们并不需要创建这个辅助类的对象
 */
public final class QueryUtils {
    ...
    private static URL createUrl(String stringUrl) {
        URL url = null;
        try {
            //这里为什么不在这里URL url，因为如果没有创建成功，则最后return会出错
     ...
     * 需要再方法开始的地方对传入的url判断，如果为空，则提前返回，不然出错。
     * 同样在JSON获取数据的时候也要判断，比如服务器错误，JSON数据为空。
     */
    ...
    private static String readFromStream(InputStream inputStream) throws IOException {
        //String 创建后就无法再赋值，但是StringBuilder可以，所以我们采用
